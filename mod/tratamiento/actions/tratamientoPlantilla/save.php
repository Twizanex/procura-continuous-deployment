<?php
/**
 * Save tratamiento plantilla entity
 *
 * @package Tratamiento
 */
...
elgg_make_sticky_form('tratamientoPlantilla');
...
if ($guid) {
	...
	if (elgg_instanceof($entity, 'object', 'tratamientoPlantilla')) {
		$tratamiento = $entity;
	} else {
		register_error(elgg_echo('tratamientoPlantilla:error:post_not_found'));
	...
	$tratamiento = new ElggTratamientoPlantilla();
	$tratamiento->subtype = 'tratamientoPlantilla';
	$new_post = TRUE;
	$tratamiento->oculto = 'no';
}
...
// PROCURA añadimos mapping de Scorm. Deshabilitado de momento...
...
//			forward($tratamiento->getURL());
			forward("tratamiento/verPlantillas");
		} else {
			forward("tratamiento/editPlantilla/$tratamiento->guid");
		}
	} else {
