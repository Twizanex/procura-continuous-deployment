...
$result = prp_add_relation($subject_user, $object_user, $relation_name);
if (!$result) {
    register_error(elgg_echo('prp:actions:relations:add:error'));
} else {
    system_message(elgg_echo('prp:actions:relations:add:ok'));
}
