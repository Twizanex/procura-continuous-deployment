...
$saludo_type = preg_replace('[\W]', '', get_input('tag'));
...
if ($friends && $owner) {
	elgg_push_breadcrumb(elgg_echo('friends'), "saludo/friends/$owner->username");
}
if ($saludo_type) {
	elgg_push_breadcrumb(elgg_echo("saludo:type:$saludo_type"));
} else {
...
if (!$owner) {
	// world saludos
	$title = elgg_echo('all') . ' ' . elgg_echo("saludo:type:$saludo_type");
} else {
	$friend_string = $friends ? elgg_echo('saludo:title:friends') : '';
	$type_string = elgg_echo("saludo:type:$saludo_type");
	$title = elgg_echo('saludo:list:title', array($owner->name, $friend_string, $type_string));
}


$sidebar = saludo_get_type_cloud($page_owner_guid, $friends);
...
if ($saludo_type) {
	...
	$params['metadata_value'] = $saludo_type;
